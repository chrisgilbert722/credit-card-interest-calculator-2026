...
import { calculateInterest } from './logic/interestCalculations';
import type { InterestInput } from './logic/interestCalculations';

function App() {
  const [values, setValues] = useState<InterestInput>({
    balance: 5000,
    apr: 22.99,
    monthlyPayment: 150
  });

  const handleChange = (field: keyof InterestInput, value: number | boolean) => {
    setValues(prev => ({ ...prev, [field]: value as number }));
  };

  const result = calculateInterest(values);
